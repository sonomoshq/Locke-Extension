...
| R-15 | One-person release process bypasses review | 3 | 4 | 12 | **Restated 2026-08-31 — the mitigation named here no longer exists.** The two-person release rule was withdrawn when publication moved to automatic-on-merge. **Restated again 2026-09-01 (Sonomos #190):** publication is now a manual dispatch of `release.yml` against `main`, so merging is no longer releasing — but the same one person may merge and then dispatch, so this risk is *unchanged in kind*. Do not read dispatch-only as a mitigation for it. What remains is pull-request review under `CODEOWNERS`, which branch protection does not enforce, so a single account with write access can still ship. Two changes would actually bite: branch protection on `main`, and required reviewers on the `store-publish` GitHub Environment. **The rating has not been re-scored** — it needs the risk owner, and it is very likely too low (`docs/security/RELEASE-POLICY.md`) | Eng/Sec |
...
| Date | Change | Author |
|---|---|---|
| 2026-09-01 | Release-model change: `release.yml` became `workflow_dispatch`-only (Sonomos #190), so merging to `main` no longer publishes and the version gate now asks whether a `v<version>` release tag exists rather than comparing against the parent commit. R-04 narrowed (reaching a store needs a deliberate dispatch, and the gate refuses any ref but `main`); R-15 explicitly NOT mitigated — one person can still merge and dispatch. Neither row was re-scored; both still need the risk owner | Eng |
